...
public final class MicrometerTelemetryAdapter {
    ...
    public void operation(String operation, String outcome) {
        registry.counter("verification.operation", "operation", bounded(operation), "outcome", bounded(outcome)).increment();
    }

    public void provider(String provider, String outcome) {
        registry.counter("verification.provider", "provider", bounded(provider), "outcome", bounded(outcome)).increment();
    }

    public void cache(String layer, String outcome) {
        registry.counter("verification.cache", "cache_layer", bounded(layer), "outcome", bounded(outcome)).increment();
    }

    public void retry(String provider, String outcome) {
        registry.counter("verification.retry", "provider", bounded(provider), "outcome", bounded(outcome)).increment();
    }
    ...
    public void latency(String operation, Duration duration) {
        registry.timer("verification.latency", "operation", bounded(operation), "latency_class", latencyClass(duration)).record(duration);
    }
    ...
    private static String latencyClass(Duration duration) {
        ...
        return millis < 100 ? "lt_100ms" : millis < 500 ? "100_500ms" : millis < 2000 ? "500ms_2s" : "gte_2s";
    }
}
