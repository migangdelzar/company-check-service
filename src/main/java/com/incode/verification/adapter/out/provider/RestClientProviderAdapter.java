...
public final class RestClientProviderAdapter implements ProviderLookupPort {
    private final RestClient client; private final ProviderType type; private final ProviderProperties.Endpoint endpoint;
    private final Duration timeout;
    public RestClientProviderAdapter(RestClient client, ProviderType type, ProviderProperties.Endpoint endpoint, Duration timeout) {
        this.client = client; this.type = type; this.endpoint = endpoint; this.timeout = timeout;
    }
    @Override public ProviderLookupResult lookup(NormalizedQuery query, ExecutionContext context) {
        ...
        } catch (ProviderException e) { return new ProviderLookupResult.Failure(e.failure); }
        catch (RestClientResponseException e) { return new ProviderLookupResult.Failure(new ProviderFailure.ClientError(e.getStatusCode().value())); }
        catch (Exception e) { return new ProviderLookupResult.Failure(new ProviderFailure.Unavailable()); }
    }
    private static final class ProviderException extends RuntimeException { final ProviderFailure failure; ProviderException(ProviderFailure f) { failure = f; } }
}
