...
public class CoordinationConfiguration {
    @Bean Cache<String, VerificationView> verificationL1Cache(CoordinationProperties p) {
        return Caffeine.newBuilder().maximumSize(p.l1MaximumSize()).build();
    }
    @Bean CoordinationPort coordinationPort(Cache<String, VerificationView> cache,
            org.springframework.data.redis.core.StringRedisTemplate redis, CoordinationProperties p) {
