...
public class ProviderConfiguration {
  ...
  RestClient freeProviderClient(ProviderProperties p) {
    return client(p.free(), p.attemptTimeout());
  }

  @Bean("premiumProviderClient")
  RestClient premiumProviderClient(ProviderProperties p) {
    return client(p.premium(), p.attemptTimeout());
  }
  ...
  private RestClient client(ProviderProperties.Endpoint e, Duration timeout) {
    ...
    CloseableHttpClient http =
        HttpClients.custom().setConnectionManager(manager).setDefaultRequestConfig(rc).build();
