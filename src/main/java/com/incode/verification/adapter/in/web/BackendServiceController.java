...
public class BackendServiceController {
    ...
    public BackendServiceController(StartVerificationUseCase starter, GetVerificationUseCase retriever) {
    ...
    public ResponseEntity<VerificationResponse> lookup(@Valid BackendServiceRequest request) {
        var result = starter.start(new StartVerificationUseCase.StartVerificationCommand(request.query()));
    ...
    private ResponseEntity<VerificationResponse> response(com.incode.verification.application.port.out.VerificationView view) {
        return ResponseEntity.ok().cacheControl(CacheControl.noCache()).body(VerificationResponse.from(view));
    }
}
