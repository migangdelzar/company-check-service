...
class BackendServiceControllerTest {
    ...
    private final MockMvc mvc = MockMvcBuilders.standaloneSetup(new BackendServiceController(starter, retriever))
    ...
    void retrievalDelegatesByVerificationId() throws Exception {
        ...
        mvc.perform(get("/backend-service/{id}", id)).andExpect(status().isOk())
                .andExpect(jsonPath("$.verificationId").value(id.toString()));
    ...
    private VerificationView view(UUID id) {
        ...
        return new VerificationView(id, "Acme", "acme", now, now.plusSeconds(600),
                com.incode.verification.domain.type.VerificationStatus.IN_PROGRESS, null, null, null, null);
    }
}
