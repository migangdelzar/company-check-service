...
class ProviderResolverTest {
    @Test void usesPremiumOnlyForApprovedFreeFailures() {
        var free = new Fixed(new ProviderLookupResult.Failure(new ProviderFailure.Unavailable()));
        var premium = new Fixed(new ProviderLookupResult.Success(java.util.List.of(), ProviderType.PREMIUM));
        var result = new ProviderResolver(free, premium).lookup(new NormalizedQuery("acme"), new ExecutionContext(UUID.randomUUID()));
        assertSame(premium.result, result);
    }
    private static final class Fixed implements ProviderLookupPort {
        final ProviderLookupResult result; Fixed(ProviderLookupResult r) { result = r; }
        public ProviderLookupResult lookup(NormalizedQuery q, ExecutionContext c) { return result; }
    }
}
